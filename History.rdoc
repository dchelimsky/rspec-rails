...
* bug fixes
  ...
  * fixed regression that was introduced in 1.2 in which controller_name failed
    to override the controller class passed to describe() (Aaron Gibralter).
    Closes #732
    * Note this is only fixed for rails >= 2.2.2
    * For rails &lt; 2.2.2, you can work around the problem by passing a string
      to describe() instead of a class.
